...
- Open interactive desktop sessions via VNC for tools like [STKO](https://asdeasoft.net/stko/) and [QGIS](https://qgis.org/), or launch visualization tools like [ParaView](https://www.paraview.org/) and [VisIt](https://visit-dav.github.io/visit-website/) on HPC nodes
...
**Virtual machines** at TACC run several applications that benefit from an interactive session without a queue wait. [MATLAB](https://www.mathworks.com/products/matlab.html), OpenSees Interactive, ADCIRC Interactive, STKO, and QGIS all run on shared VMs. STKO and QGIS provide a full graphical desktop through VNC. VMs share hardware across users, so performance varies under load, and they are best for lightweight tasks and quick tests.
...
Interactive workflows involve writing code, running it, inspecting results, modifying inputs, and iterating. The feedback loop is tight, often seconds between runs. JupyterHub, the VM-based sessions (MATLAB, OpenSees Interactive, QGIS), and VNC desktop environments (STKO, QGIS) all support this mode.
...
| Quick interactive session with a GUI tool | VM with VNC (interactive) | Exploring spatial data in QGIS, building an OpenSees model in STKO |
